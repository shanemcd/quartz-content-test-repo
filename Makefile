...
IMAGE_TAG ?= latest
QUARTZ_SERVER_PORT ?= 5005
QUARTZ_WEBSOCKET_PORT ?= 3001

QUARTZ_FORK ?= jackyzha0
QUARTZ_REF ?= v4
QUARTZ_REPO_PATH ?=
...
ifneq ($(QUARTZ_REPO_PATH),)
COMMON_MOUNTS += -v $(QUARTZ_REPO_PATH):/opt/quartz:Z
endif
...
scratchpad: check_image_uptodate
	$(CONTAINER_RUNTIME) run --rm -ti \
		...
		bash -c 'SUPPRESS_WARNING=yes QUARTZ_BUILD_OPTS="--serve --port=$(QUARTZ_SERVER_PORT) --directory=/repo/content" make -B -f /repo/Makefile public'
...
$(IMAGE_ID_FILE): .generated Dockerfile quartz.config.ts
	$(CONTAINER_RUNTIME) build -t $(SCRATCHPAD_IMAGE_NAME):$(IMAGE_TAG) --build-arg QUARTZ_FORK=$(QUARTZ_FORK) --build-arg QUARTZ_REF=$(QUARTZ_REF) .
